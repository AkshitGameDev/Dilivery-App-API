...
const OrderSchema = z.object({
  ...
  job: JobHintSchema,                 // ← optional hint for auto-creating a job on first insert
});
...
r.post('/import-orders', async (req, res) => {
  try {
    ...
    const { platform, order, job } = parsed.data;

    // 2) Basic idempotency (10 min)
    const idem = req.get('Idempotency-Key');
    if (idem) {
      const key = `idem:orders:${idem}`;
      const set = await redis.set(key, '1', 'NX', 'EX', 600);
      if (set !== 'OK') {
        return res.status(409).json({ error: 'duplicate_request' });
      }
    }

    // 3) Hash to detect identical replays
    const hash = payloadHash(parsed.data);

    // 4) Short-circuit on exact duplicate
    ...
    // 6) Optional glue to Q2: auto-create one OPEN job on first creation
    let jobCreated = false;
    if (!wasExisting) {
      const hints = job ?? {};
      const coordsArePresent =
        hints.pickupLat != null && hints.pickupLng != null;

      // Only create when we have pickup coords (or adapt here to your rules)
      if (coordsArePresent) {
        await prisma.job.create({
          data: {
            orderId: up.id,
            status: 'OPEN',
            pickupLat: hints.pickupLat,
            pickupLng: hints.pickupLng,
            dropLat:   hints.dropLat ?? null,
            dropLng:   hints.dropLng ?? null,
          },
        });
        jobCreated = true;
      }
